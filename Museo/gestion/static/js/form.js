const rut = document.getElementById("txtCodigo");
...
function validacionForm() {
  ...
  if (rut.value.length < 1 || rut.value.trim() == "") {
    mostrarMensajeError("txtCodigo", "rut no valido*");
    condicion = false;
  }
  if (nombre.value.length < 1 || nombre.value.trim() == "") {
    mostrarMensajeError("txtNombre", "Nombre no valido*");
    condicion = false;
  }
